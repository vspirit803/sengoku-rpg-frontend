      ...
      <v-tab-item value="equipment">
        <div class="items-container">
          ...
          <template v-if="equipments.length < size">
            <Item v-for="eachIndex of size - equipments.length" :key="getNullSymbol(eachIndex)" :item="null" />
          </template>
        </div>
      </v-tab-item>
      <v-tab-item value="system">
        <div class="items-container">
          ...
          <template v-if="systemItems.length < size">
            <Item v-for="eachIndex of size - systemItems.length" :key="getNullSymbol(eachIndex)" :item="null" />
          </template>
        </div>
      </v-tab-item>
    </v-tabs>
...
<script lang="ts">
import { computed, defineComponent, onActivated, Ref, ref } from '@vue/composition-api';
import { ItemBase, ItemSystem, ItemType } from 'sengoku-rpg-core';
